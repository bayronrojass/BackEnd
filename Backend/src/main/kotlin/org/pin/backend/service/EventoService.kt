...
class EventoService(
    ...
    fun crearEvento(casaId: Long, request: EventoRequestDTO): Evento {

        val casa = casaRepo.findById(casaId)
            .orElseThrow { IllegalArgumentException("Casa no encontrada con ID: $casaId") }

        val creadoPor = usuarioRepo.findById(request.creadoPor)
            .orElseThrow { IllegalArgumentException("Usuario no encontrado con ID: ${request.creadoPor}") }

        val asistentes = mutableListOf<Usuario>().apply {
            add(creadoPor)
        }
