...
class CatalogoService(
    ...
    data class CatalogoDTO(val id: Long, val nombre: String, val categoria: String, val iconoKey: String)

    fun buscarPredictivo(query: String): List<CatalogoDTO> {
        return repository.buscarPorNombrePredictivo(query).map {
            CatalogoDTO(it.id!!, it.nombre, it.categoria, it.iconoKey)
        }
    }

    fun obtenerRapidos(): List<CatalogoDTO> {
        return repository.findTop12ByOrderByIdAsc().map {
            CatalogoDTO(it.id!!, it.nombre, it.categoria, it.iconoKey)
        }
    }
