...
class CatalogoController(
    ...
    @GetMapping("/buscar")
    fun buscarProductos(@RequestParam("q") query: String): ResponseEntity<List<CatalogoService.CatalogoDTO>> {
    ...
    fun obtenerPopulares(): ResponseEntity<List<CatalogoService.CatalogoDTO>> {
        return ResponseEntity.ok(catalogoService.obtenerRapidos())
    }
}
